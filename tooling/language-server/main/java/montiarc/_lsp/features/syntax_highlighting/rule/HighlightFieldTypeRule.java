...
public class HighlightFieldTypeRule implements TokenClassificationRule {

  @Override
  public boolean matches(Token token) {
    return token.tokenPathMatches(".*.arcFieldDeclaration.mCObjectType.*")
      && token.isNameToken();
  }

  @Override
  public Optional<TokenClassification> apply(Token token) {
    return Optional.of(new TokenClassification(SemanticTokenTypesWrapper.Type));
  }
}
