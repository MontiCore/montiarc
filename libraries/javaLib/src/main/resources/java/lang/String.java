    ...
    implements java.io.Serializable, Comparable<String>, CharSequence
{
  ...
  public String(byte[] ascii, int hibyte, int offset, int count) {
    ...
    if (hibyte == 0) {
      ...
      for (int ii = count ; ii-- > 0 ;) {
        value[ii] = (char) (hibyte | (ascii[ii + offset] & 0xff));
      }
    }
    this.offset = 0;
    this.count = count;
  ...
  public String toUpperCase(Locale locale) {
    ...
    int srcChar;
    int srcCountn;
    for (int i = firstLower; i < count; i += srcCountn) {
      srcChar = (int)value[offset+i];
      if ((char)srcChar >= Character.MIN_HIGH_SURROGATE &&
          (char)srcChar <= Character.MAX_HIGH_SURROGATE) {
        srcChar = codePointAt(i);
        srcCountn = Character.charCount(srcChar);
      } else {
        srcCountn = 1;
      }
      if (localeDependent) {
      ...
      if ((upperChar == Character.ERROR) ||
        ...
        if (upperChar == Character.ERROR) {
          ...
          resultOffset += Character.toChars(upperChar, result, i + resultOffset) - srcCountn;
          continue;
        } else {
        ...
        if (mapLen > srcCountn) {
          char[] result2 = new char[result.length + mapLen - srcCountn];
          System.arraycopy(result, 0, result2, 0,
              i + resultOffset);
          result = result2;
        }
