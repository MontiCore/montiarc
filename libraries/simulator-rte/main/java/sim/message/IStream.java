...
public interface IStream<T> extends List<TickedMessage<T>> {
  ...
  /**
   * Counts the occurrence from the given message in the history using the equal() method from the contained messages to
   * compare them.
   *
  ...
  /**
   * Returns a copy from this stream, that only contains elements that are in e.
   *
  ...
  /**
   * Returns all unsent messages. Note: The messages in the buffer will be received in a future time unit. So this
   * method must only be used by the simulator to display unsent messages.
   *
  ...
  /**
   * @return the trace/history from this stream, where the youngest message is the last list element and the oldest is
   * the first. Only delivered messages are in the history.
   */
  IStream<T> getHistory();

  /**
   * @param n length
   * @return the last n messages from the history. If history is shorter then n, it returns the history.
   */
   ...
   * @return all messages in the time interval from begin to end (including begin and end)
   */
  ...
  /**
   * @param message message to find
   * @return the time interval in which the given <b>message</b> has been transmitted in this stream. If <b>message
   * </b>
   * is not transmitted in this stream, the return value will be -1.
   */
  int firstTimeIntervalOf(T message);

  /**
   * @param message message to find
   * @return an array of time intervals in which the given <b>message</a> has been transmitted. The array is empty, if
   * the message has not been transmitted in this stream.
   */
  ...
  /**
   * Checks, if this is the prefix from the given stream. In other words: the given stream starts with this stream. Only
   * the transmitted history (@see IStream.getHistory()) of both streams is compared.
   *
  ...
  /**
   * Creates a copy of this stream from fromIndex to toIndex. Respects history and buffer from this stream.
   *
  ...
  /**
   * @return a shallow copy of this streams with the same history/buffer. Copy contains links to the same message
