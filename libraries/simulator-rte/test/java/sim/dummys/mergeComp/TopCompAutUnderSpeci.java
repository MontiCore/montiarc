...
public class TopCompAutUnderSpeci extends TopComp implements Serializable {
  ...
  public List<ComponentState> findPossIn(int m) {
    ...
    if (currentState.getCurrentState() == AutomataState.ONE) {
      ...
      if (m > 1) {
        Map<IPort, List<TickedMessage>> outmsg = new HashMap<>();
        outmsg.put((IPort) gettOut(), List.of(Message.of(m + 1)));
        posscompstates.add(new ComponentState(AutomataState.ONE, null, (IPort) gettIn(), Message.of(m), outmsg, getComponentName(), "m>1"));
      }

      if (m < 0) {
        Map<IPort, List<TickedMessage>> outmsg = new HashMap<>();
        outmsg.put((IPort) gettOut(), List.of(Message.of(m)));
        posscompstates.add(new ComponentState(AutomataState.ONE, null, (IPort) gettIn(), Message.of(m), outmsg, getComponentName(), "m<0"));
      }
    }
