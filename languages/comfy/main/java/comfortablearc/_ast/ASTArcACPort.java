...
public class ASTArcACPort extends ASTArcACPortTOP {

  /**
   * If the auto-connect mode is set to port, then the source and target are
  ...
  public boolean matches(@NotNull PortSymbol source, @NotNull PortSymbol target, @NotNull SymTypeRelations tr) {
    ...
    return source.getName().equals(target.getName()) && matches(source.getType(), target.getType(), tr);
  }
