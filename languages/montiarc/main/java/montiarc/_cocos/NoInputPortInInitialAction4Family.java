...
import montiarc._cocos.util.ASTSCStateCollector;
...
public class NoInputPortInInitialAction4Family implements ArcBasisASTArcComponentTypeCoCo {
  ...
  public void check(@NotNull ASTArcComponentType node) {
    ...
    Map<ASTSCState, BoolExpr> stateConditions;
    ...
    stateConditions = new HashMap<>();

    MontiArcTraverser nodeTraverser = MontiArcMill.traverser();
    ASTSCStateCollector stateCollector = new ASTSCStateCollector();
    nodeTraverser.add4SCBasis(stateCollector);
    node.accept(nodeTraverser);

    for (ASTSCState state : stateCollector.getExpressions()) {
      stateConditions.put(state, ctx.mkTrue());
    }
    ...
    if (node instanceof ASTVariableArcFullVariantComponentType) {
      ...
      for (VariableArcVariationPoint variationPoint : variationPoints) {
        ...
        ASTSCStateCollector varifStateCollector = new ASTSCStateCollector();
        varifTraverser.add4SCBasis(varifStateCollector);

        for (ASTArcStatechart statechart : statecharts) {
          statechart.accept(varifTraverser);
          for (ASTSCState state : varifStateCollector.getExpressions()) {
            stateConditions.put(state, variationExpr);
          }
          varifStateCollector.clearExpressions();
        }
      }
    } else {
      ExpressionSet mainConstraintSet = null;
    ...
    for (Map.Entry<ASTSCState, BoolExpr> stateEntry : stateConditions.entrySet()) {

      // Step 1: Check if action can be active
      List<BoolExpr> stateEntryExpressionList = new ArrayList<>(List.of(featureConstraints, stateEntry.getValue()));
      if (ExpressionSolverService.solve(stateEntryExpressionList) == Status.UNSATISFIABLE)
        continue;

      stateEntryExpressionList.clear();
      ...
      if (!variableNames.isEmpty()) {
        for (String variableName : variableNames) {
          ...
          for (Map.Entry<ASTArcPort, BoolExpr> entry : possiblePorts) {
            stateEntryExpressionList.clear();
            stateEntryExpressionList.addAll(List.of(featureConstraints, ctx.mkAnd(stateEntry.getValue(), entry.getValue())));
            if (ExpressionSolverService.solve(stateEntryExpressionList) == Status.SATISFIABLE) {
              if (shadowingFields.isEmpty()) {
                SourcePosition sourcePosition = stateEntry.getKey().get_SourcePositionStart();
                Log.error(IN_PORT_REF_IN_INVALID_CONTEXT.format(variableName, stateEntry.getKey().getSCSAnte()), sourcePosition);
              }
            }
          }
        }
