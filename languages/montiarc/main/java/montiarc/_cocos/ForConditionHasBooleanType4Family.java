...
import montiarc._cocos.util.ASTForStatementCollector;
...
public class ForConditionHasBooleanType4Family implements ArcBasisASTArcComponentTypeCoCo {
  ...
  public void check(@NotNull ASTArcComponentType node) {
    ...
    Map<ASTForStatement, BoolExpr> forStatementConditions;
    ...
    ASTForStatementCollector forStatementCollector = new ASTForStatementCollector();
    nodeTraverser.add4MCCommonStatements(forStatementCollector);
    node.accept(nodeTraverser);

    for (ASTForStatement forStatement : forStatementCollector.getExpressions()) {
    ...
    if (node instanceof ASTVariableArcFullVariantComponentType) {
      ...
      for (VariableArcVariationPoint variationPoint : variationPoints) {
        ...
        ASTForStatementCollector varifForStatementCollector = new ASTForStatementCollector();
        varifTraverser.add4MCCommonStatements(varifForStatementCollector);
        ...
        for (ASTForStatement forStatement : varifForStatementCollector.getExpressions()) {
          forStatementConditions.put(forStatement, variationExpr);
        }
        varifForStatementCollector.clearExpressions();
      }
    } else {
      if (node.isPresentSymbol())
    ...
    for (Map.Entry<ASTForStatement, BoolExpr> forStatementEntry : forStatementConditions.entrySet()) {
      ...
      for (ASTExpression possibleExpression : possibleExpressions) {
        ...
        if (!SymTypeRelations.isBoolean(result)) {
          Log.error(MCError.FOR_CONDITION_NOT_BOOLEAN.format(), forStatementEntry.getKey().get_SourcePositionStart());
        }
      }
    }
