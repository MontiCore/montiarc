...
import montiarc._cocos.util.ASTSCEntryActionCollector;
...
import static arcautomaton._cocos.NoInputPortInEntryAction.CONTEXT;
import static montiarc.util.ArcError.IN_PORT_REF_IN_INVALID_CONTEXT;

public class NoInputPortInEntryAction4Family implements ArcBasisASTArcComponentTypeCoCo {
  ...
  public void check(@NotNull ASTArcComponentType node) {
    ...
    Map<ASTSCEntryAction, BoolExpr> actionConditions;
    ...
    nodeTraverser.add4SCActions(entryActionCollector);
    node.accept(nodeTraverser);

    for (ASTSCEntryAction action : entryActionCollector.getExpressions()) {
    ...
    if (node instanceof ASTVariableArcFullVariantComponentType) {
      ...
      for (VariableArcVariationPoint variationPoint : variationPoints) {
        ...
        ASTSCEntryActionCollector varifActionCollector = new ASTSCEntryActionCollector();
        ...
        for (ASTArcStatechart statechart : statecharts) {
          ...
          for (ASTSCEntryAction action : varifActionCollector.getExpressions()) {
    ...
    for (Map.Entry<ASTSCEntryAction, BoolExpr> actionEntry : actionConditions.entrySet()) {
      ...
      List<BoolExpr> actionEntryExpressionList = new ArrayList<>();
      // Step 1: Check if action can be active
      actionEntryExpressionList.addAll(List.of(featureConstraints, actionEntry.getValue()));
