...
public class MontiArcTypeCheckTest extends AbstractArcTypeCalculatorTest {
  ...
  public void setUpMessageType() {
    ...
    FunctionSymbol getHeader = MontiArcMill.functionSymbolBuilder().setName("getHeader")
      ...
      .setType(SymTypeExpressionFactory.createTypeObject(getScope().resolveType("String").orElseThrow()))
  ...
  public void setUpTrafoBuilderType() {
    ...
    List<SymTypeExpression> trafoArgs = Arrays.asList(
      SymTypeExpressionFactory.createTypeObject(getScope().resolveType("Student").orElseThrow()),
      SymTypeExpressionFactory.createTypeObject(getScope().resolveType("Teacher").orElseThrow()));
    FunctionSymbol build = MontiArcMill.functionSymbolBuilder().setName("build")
      .setAccessModifier(AccessModifier.ALL_INCLUSION)
      .setType(SymTypeExpressionFactory.createGenerics(getScope().resolveType("Trafo").orElseThrow(), trafoArgs))
  ...
  public void setUpGenericFields() {
    SymTypeExpression bufferArg = SymTypeExpressionFactory.createTypeObject(getScope().resolveType("String").orElseThrow());
    FieldSymbol buffer = ArcBasisMill.fieldSymbolBuilder().setName("strBuffer")
      .setType(SymTypeExpressionFactory.createGenerics(getScope().resolveType("Buffer").orElseThrow(), bufferArg)).build();
    SymTypeExpression buffer2Arg = SymTypeExpressionFactory.createTypeObject(getScope().resolveType("Role").orElseThrow());
    FieldSymbol buffer2 = ArcBasisMill.fieldSymbolBuilder().setName("roleBuffer")
      .setType(SymTypeExpressionFactory.createGenerics(getScope().resolveType("Buffer").orElseThrow(), buffer2Arg)).build();
    SymTypeExpression storageArg = SymTypeExpressionFactory.createTypeObject(getScope().resolveType("Message").orElseThrow());
    FieldSymbol storage = ArcBasisMill.fieldSymbolBuilder().setName("msgStorage")
      .setType(SymTypeExpressionFactory.createGenerics(getScope().resolveType("Storage").orElseThrow(), storageArg)).build();
    List<SymTypeExpression> ma2javaArgs = Arrays.asList(
      SymTypeExpressionFactory.createTypeObject(getScope().resolveType("Student").orElseThrow()),
      SymTypeExpressionFactory.createTypeObject(getScope().resolveType("Teacher").orElseThrow()));
    FieldSymbol trafo = ArcBasisMill.fieldSymbolBuilder().setName("ma2java")
      .setType(SymTypeExpressionFactory.createGenerics(getScope().resolveType("Trafo").orElseThrow(), ma2javaArgs)).build();
  ...
  public void setUpMsgFields() {
    FieldSymbol msg = ArcBasisMill.fieldSymbolBuilder().setName("msg")
      .setType(SymTypeExpressionFactory.createTypeObject(getScope().resolveType("Message").orElseThrow())).build();
    SymbolService.link(this.getScope(), msg);
  }

  public void setUpTrafoBuilderFields() {
    FieldSymbol builder = ArcBasisMill.fieldSymbolBuilder().setName("trafoBuilder")
      .setType(SymTypeExpressionFactory.createTypeObject(getScope().resolveType("TrafoBuilder").orElseThrow())).build();
    SymbolService.link(this.getScope(), builder);
  }
  ...
  public void shouldCalculateObjectTypeFromMethodCalls(@NotNull String expression,
    ...
    Assertions.assertFalse(result.isPrimitive());
    Assertions.assertFalse(result.isGenericType());
  ...
  public void shouldCalculateObjectTypeFromGenericMethodCalls(@NotNull String expression,
    ...
    Assertions.assertFalse(result.isObscureType());
    Assertions.assertFalse(result.isPrimitive());
    Assertions.assertTrue(result.isGenericType());
    Assertions.assertFalse(result.isTypeVariable());
    Assertions.assertTrue(!(result.getTypeInfo() instanceof OOTypeSymbolSurrogate) ||
      !(((OOTypeSymbolSurrogate) result.getTypeInfo()).lazyLoadDelegate() instanceof OOTypeSymbolSurrogate));
    Assertions.assertEquals(expectedType, result.print());
  ...
  public void shouldCalculatePrimitiveTypeFromAssignment(@NotNull String expression,
    ...
    //Then
    Assertions.assertFalse(result.isObscureType());
    Assertions.assertTrue(result.isPrimitive());
    Assertions.assertFalse(result.isGenericType());
    Assertions.assertFalse(result.isTypeVariable());
    Assertions.assertTrue(!(result.getTypeInfo() instanceof OOTypeSymbolSurrogate) ||
      !(((OOTypeSymbolSurrogate) result.getTypeInfo()).lazyLoadDelegate() instanceof OOTypeSymbolSurrogate));
