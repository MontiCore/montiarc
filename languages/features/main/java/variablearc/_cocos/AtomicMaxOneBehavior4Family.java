...
public class AtomicMaxOneBehavior4Family implements ArcBasisASTArcComponentTypeCoCo {
  ...
  public void check(@NotNull ASTArcComponentType node) {
    ...
    if (node instanceof ASTVariableArcFullVariantComponentType) {
      ...
      ArrayList<ASTArcBehaviorElement> mainBehaviors = (ArrayList<ASTArcBehaviorElement>) node.getBody().getArcElementList().stream().filter(e -> e instanceof ASTArcBehaviorElement).map(k -> (ASTArcBehaviorElement) k).collect(Collectors.toList());
