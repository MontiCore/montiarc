  ...
  public void check(ASTAutomaton node) {
    ...
    for (ASTTransition transition : node.getTransitions()) {
      ...
      if (!names.contains(sourceName)) {
        Log.error("xMA026 The source " + sourceName + " in " + sourceName + "->" + targetName + " was not defined as a state.", transition.get_SourcePositionStart());
      }
      if (!names.contains(targetName)) {
        Log.error("xMA027 The target " + targetName + " in " + sourceName + "->" + targetName + " was not defined as a state.", transition.get_SourcePositionStart());
      }
    }
  }
