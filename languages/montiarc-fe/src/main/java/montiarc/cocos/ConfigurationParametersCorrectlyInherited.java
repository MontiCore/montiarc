 ...
 * Considered valid are components which specify at least as many configuration
 * parameters as the extended component.
 * Furthermore, if `n` specifies the
 * number of parameters of the extended component, the types of the first `n`
 * parameters of the extending component have to match those of it's super
 * component. This means that the names of the parameters do not have to
 * match those of the super component.
 * If the extended component has optional parameters with default values,
 * then all new parameters of the inheriting component have to be optional
 * as well.
 *
...
public class ConfigurationParametersCorrectlyInherited implements MontiArcASTComponentCoCo {
  ...
  public void check(ASTComponent component) {
    ...
    final Optional<? extends Symbol> componentSymbolOpt
        = component.getSymbol();
    if (componentSymbolOpt.isPresent()
            && componentSymbolOpt.get() instanceof ComponentSymbol) {
      ComponentSymbol componentSymbol
          = (ComponentSymbol) componentSymbolOpt.get();
      ...
      final Optional<ComponentSymbolReference> superComponentOpt
          = componentSymbol.getSuperComponent();
      ...
      if (superComponentOpt.isPresent()) {
        ...
        for (int paramIndex = 0; paramIndex < Math.min(numInheritedParams, configParameters.size());
             paramIndex++) {
          ...
          if (!TypeCompatibilityChecker
                   .doTypesMatch(superParameterType, paramType)) {
            Log.error(
