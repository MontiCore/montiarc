 ...
 * (p.57)
 */
public class NamesCorrectlyCapitalized implements ArcASTComponentCoCo {
  ...
  public void check(@NotNull ASTComponent node) {
    ...
    final List<ASTArcParameterDeclaration> declarations = node.getHead().getParameterList();
    for (ASTArcParameterDeclaration declaration : declarations) {
      if (!Character.isLowerCase(declaration.getArcParameter().getName().charAt(0))) {
        Log.error(String.format(ArcError.PARAMETER_LOWER_CASE.toString(),
          declaration.getArcParameter().getName(), node.getName()),
          declaration.get_SourcePositionStart());
      }
    }
