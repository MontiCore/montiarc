 ...
 */
public class ConfigurationParametersCorrectlyInherited implements ArcASTComponentCoCo {
  ...
  public void check(@NotNull ASTComponent node) {
    ...
    if (component.isPresentParentComponent() && component.getParent().loadSymbol().isPresent()) {
      ComponentSymbol parent = component.getParentInfo();
      ...
      for (int i = 0; i < Math.min(parentParameters.size(), parameters.size()); i++) {
        ...
        if (!TypeCheck.compatible(paramType, superParameterType)) {
          Log.error(
            ...
            node.getHead().getParameterList().get(i).get_SourcePositionStart());
        }
  ...
  private boolean hasParameterDefaultValue(ComponentSymbol symbol, int position) {
    if (symbol.isPresentAstNode()) {
      final ASTComponent astNode = symbol.getAstNode();
      return astNode.getHead().getParameterList().get(position).isPresentDefaultValue();
    }
    return false;
  }
}