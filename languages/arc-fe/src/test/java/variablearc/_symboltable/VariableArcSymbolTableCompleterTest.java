...
public class VariableArcSymbolTableCompleterTest extends AbstractTest {
  ...
  public void shouldVisitComponentInstanceSymbol() {
    ...
    VariableSymbol parameter = VariableArcMill.variableSymbolBuilder().setName("a").setEnclosingScope(scope).build();
    scope.add(parameter);
    ArcFeatureSymbol feature = VariableArcMill.arcFeatureSymbolBuilder().setName("b").build();
    scope.add(feature);
    ...
    ComponentInstanceSymbol symbol = VariableArcMill.componentInstanceSymbolBuilder().setArguments(Arrays.asList(
        bindings.get(0),
    ...
    Assertions.assertEquals(typeExpr.getTypeInfo(), symbol.getType().getTypeInfo());
   ...
   */
  @ParameterizedTest
  @MethodSource("visitComponentInstanceSymbolExpectedExceptionProvider")
  public void visitShouldTrowException(@Nullable ComponentInstanceSymbol symbol,
      @NotNull Class<Exception> expected) {
    Preconditions.checkNotNull(expected);
    // When && Then
    Assertions.assertThrows(expected, () -> this.getCompleter().visit(symbol));
  }

  protected static Stream<Arguments> visitComponentInstanceSymbolExpectedExceptionProvider() {
    ComponentInstanceSymbol symbol1 = VariableArcMill.componentInstanceSymbolBuilder().setName("c1").build();

    return Stream.of(
        Arguments.of(null, NullPointerException.class),
        Arguments.of(symbol1, IllegalStateException.class)
    );
  }
}
