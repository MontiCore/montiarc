...
public class FieldInitOmitPortReferences implements ArcBasisASTComponentTypeCoCo {
  ...
  public FieldInitOmitPortReferences() {
    this.portRefExtractor = new PortReferenceExtractor4ExpressionBasis();
  }

  public FieldInitOmitPortReferences(@NotNull IPortReferenceInExpressionExtractor portRefExtractor) {
  ...
  public void check(@NotNull ASTComponentType astComp) {
    ...
    fields.removeAll(comp.getParameterList()); // ComponentTypeSymbol::getFields also returns parameters
    ...
    for (VariableSymbol field : fields) {
      if (field.isPresentAstNode() && field.getAstNode() instanceof ASTArcField) {
        ...
        for (PortReference illegalPortRef : foundPortReferences.keySet()) {
          ...
          Log.error(ArcError.PORT_REF_FIELD_INIT.toString(), illegalPortRefPosition);
        }
      }
    }
  }
