...
<#if portSym.isOutgoing()>
    public void set${portSym.getName()?cap_first}(${glex.getGlobalVar("IPort")}<${generatorHelper.printType(portSym.getTypeReference())}> port) {
    <#if compSym.isAtomic()>
    <#assign portName = "${portSym.getName()?uncap_first}">        
        if (this.${portName} == null) {
            ...
            ((${glex.getGlobalVar("IOutSimPort")}<${generatorHelper.printType(portSym.getTypeReference())}>) this.${portName}).addReceiver(port);
        }
