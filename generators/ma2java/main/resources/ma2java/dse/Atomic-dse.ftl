...
<#macro printSetUp comp>
  public void setUp() {
    ...
    <#list comp.getPorts() as port>
      this.${port.getName()} = new montiarc.rte.timesync.<#if port.isStronglyCausal()>Delay<#elseif port.isOutgoing()>Out<#else>In</#if>Port<>(!this.getInstanceName().isBlank() ? this.getInstanceName() + "." + "${port.getName()}" : "${port.getName()}");
    </#list>
  }
</#macro>
