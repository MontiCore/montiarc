...
public class MAADynRecGeneratorScript extends Script implements GroovyRunner {
  
  protected static final String[] DEFAULT_IMPORTS = {};
  
  protected static final String LOG = "MAADynRecScript";
  ...
  public void run(String script, Configuration configuration) {
    GroovyInterpreter.Builder builder = GroovyInterpreter.newInterpreter()
        .withScriptBaseClass(MAADynRecGeneratorScript.class)
        .withImportCustomizer(new ImportCustomizer().addStarImports(DEFAULT_IMPORTS));
    
    // configuration
    MAADynRecConfiguration config = MAADynRecConfiguration
        .withConfiguration(configuration);
    
    // we add the configuration object as property with a special property
    // name
    builder.addVariable(MAADynRecConfiguration.CONFIGURATION_PROPERTY, config);
    ...
    builder.addVariable(MAADynRecConfiguration.Options.MODELPATH.toString(),
        config.getModelPath());
    builder.addVariable(MAADynRecConfiguration.Options.OUT.toString(),
        config.getOut());
    builder.addVariable(MAADynRecConfiguration.Options.HANDWRITTENCODEPATH.toString(),
        config.getHWCPath());
  ...
  public void generate(File modelPath, File targetFilepath, File hwcPath) {
    new DynamicMontiArcGeneratorTool().generate(modelPath, targetFilepath, hwcPath);
  }
