${tc.signature("symbol", "package", "comments", "helper", "existsHWC", "formalTypeParameters")}
<#assign genHelper = helper>

<#if !symbol.isInnerComponent()>
<#-- Copyright -->
...
import de.montiarc.simplegenerator.datatypes.Port;
/**
 * ${comments}
 * <br><br>
...
<#if existsHWC>abstract</#if> public <#if symbol.isInnerComponent()> static </#if> class ${symbol.getName()}<#if existsHWC>TOP</#if> ${formalTypeParameters} <#t>
{
  ...
  <#list symbol.getSubComponents() as sub>
    ...
    <#if sub.getComponentType().isInnerComponent()>
    protected ${helper.printType(sub.getComponentType())}TOP ${sub.getName()};
    
    public ${helper.printType(sub.getComponentType())}TOP get${subNameCap}() {
        return ${sub.getName()};
    }
    
    public void set${subNameCap}(${helper.printType(sub.getComponentType())}TOP ${sub.getName()}) {
    ...
    public void setUp(){
      ...
      <#else>
        <#list symbol.getSubComponents() as sub>
          <#if sub.getComponentType().isInnerComponent()>
          this.${sub.getName()} = new ${helper.printInnerComponent(sub.getComponentType(), symbol.getName())}();
          <#else>
    ...
    <#list symbol.getInnerComponents() as sub>
       ${tc.includeArgs("templates.component.Component", [sub, package, "", helper, true, ""])}
    </#list>
