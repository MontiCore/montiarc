...
class ByteForwardTest {
  ...
  void testIO(@NotNull List<Byte> input,
    ...
    // Given
    ByteForwardComp sut = new ByteForwardCompBuilder().setScheduler(new InstantSchedule()).setName("sut").build();
    ...
    // Then
    List<Byte> actualAsBytes = this.actual_outVal.getAllValues().stream()
  ...
  static Stream<Arguments> io() {
    return Stream.of(
      ...
      Arguments.of(
        List.<Byte>of((byte) 1, (byte) 2, Byte.MAX_VALUE),
        List.<Byte>of((byte) 1, (byte) 2, Byte.MAX_VALUE)
      )
    );
  }
