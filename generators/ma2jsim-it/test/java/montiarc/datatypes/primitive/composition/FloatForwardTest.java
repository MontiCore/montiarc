...
class FloatForwardTest {
  ...
  void testIO(@NotNull List<Float> input,
    ...
    // Given
    FloatForwardComp sut = new FloatForwardCompBuilder().setScheduler(new InstantSchedule()).setName("sut").build();
    ...
    // Then
    List<Float> actualAsBytes = this.actual_outVal.getAllValues().stream()
  ...
  static Stream<Arguments> io() {
    return Stream.of(
      ...
      Arguments.of(
        List.<Float>of(1.5f, 2.25f, Float.MAX_VALUE, Float.MIN_NORMAL, Float.POSITIVE_INFINITY, Float.NaN, -0.0f),
        List.<Float>of(1.5f, 2.25f, Float.MAX_VALUE, Float.MIN_NORMAL, Float.POSITIVE_INFINITY, Float.NaN, -0.0f)
      )
    );
  }
