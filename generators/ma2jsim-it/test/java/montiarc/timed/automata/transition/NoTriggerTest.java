...
class NoTriggerTest {
  ...
  void testIO(@NotNull List<Message<Integer>> input,
    ...
    // Given
    NoTriggerComp sut = new NoTriggerCompBuilder().setScheduler(new InstantSchedule()).setName("sut").build();
  ...
  static Stream<Arguments> io() {
    return Stream.of(
      Arguments.of(
        List.of(msg(1), msg(2), tk(), msg(-2), tk(), msg(0), msg(-1), tk(), tk()),
        List.of(msg(ON), tk(), msg(ON), tk(), msg(ON), tk(), msg(ON), tk())
      ),
      Arguments.of(
        List.of(msg(1), msg(2), tk(), tk(), msg(-2), tk(), tk(), tk()),
        List.of(msg(ON), tk(), msg(ON), tk(), msg(ON), tk(), msg(ON), tk(), msg(ON), tk())
      )
    );
  }
}
