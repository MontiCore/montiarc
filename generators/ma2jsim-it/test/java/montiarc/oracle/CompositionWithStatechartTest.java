...
@JSimTest
class CompositionWithStatechartTest {
  ...
  void testIO(@NotNull List<Message<OnOff>> input,
    ...
    // Given
    CompositionWithStatechartComp sut =
      new CompositionWithStatechartCompBuilder()
        .setName("sut")
  ...
  static Stream<Arguments> inputExpectedOutputAndOracleProvider() {
    return Stream.of(
      ...
      Arguments.of(
        ...
        OracleFactory.withDefaultStrategy(preferFirst())
          .set4Comp("sut.sub", preferLast())
      ),
      Arguments.of(
        List.of(msg(ON), tk(), msg(ON),  tk(), msg(ON), tk()),
        List.of(msg(1), tk(), msg(2), tk(), msg(1), tk()),
        OracleFactory.withDefaultStrategy(preferFirst())
          .set4Comp("sut.sub", preferUnexplored(preferFirst()))
      )
    );
  }
}
