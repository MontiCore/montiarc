...
class DynamicComponentGenerator {
  ...
  def generateAll(File targetPath, File hwc, ComponentSymbol comp) {
    ...
    if (comp.isAtomic) {
      toFile(targetPath, "Dynamic" + comp.name, generateAtomicComponent(comp));
      if(!existsHWCClass) {
        toFile(targetPath, comp.name + "Impl", generateBehaviorImplementation(comp));
      }
    } else {
      toFile(targetPath, "Dynamic" + comp.name, generateComposedComponent(comp));
    }

    if (comp.getStereotype().containsKey("deploy")) {
      toFile(targetPath, "DynamicDeploy" + comp.name, generateDeploy(comp));
    }
  ...
  def generateDeploy(ComponentSymbol comp) {
    ...
    return '''
      ...
      import de.montiarcautomaton.runtimes.timesync.delegation.LoaderManager;
      ...
      public class DynamicDeploy«name» {
        ...
        final static String STORE_DIR = "";
        final static String CLASS_DIR = "";
          
        public static void main(String[] args) {
          final LoaderManager loman = new LoaderManager();
          final Dynamic«name» cmp = new Dynamic«name»();
          
          cmp.setUp();
          cmp.setLoaderConfiguration(«name», STORE_DIR, CLASS_DIR, loman)
          cmp.init();
  ...
  def generateAtomicComponent(ComponentSymbol comp) {
    ...
    return '''
      ...
      import de.montiarcautomaton.runtimes.timesync.delegation.IDynamicComponent;
      import de.montiarcautomaton.runtimes.timesync.delegation.Port;
      import de.montiarcautomaton.runtimes.timesync.delegation.FileSystemLoader;
      import de.montiarcautomaton.runtimes.timesync.delegation.LoaderManager;
      import de.montiarcautomaton.runtimes.timesync.implementation.IComputable;
      import de.montiarcautomaton.runtimes.Log;

      
      public class Dynamic«comp.name»«printGenerics(comp)»      
      ...
      implements IDynamicComponent {
        ...
        public Dynamic«comp.name»(«FOR param : comp.configParameters SEPARATOR ','» «helper.getParamTypeName(param)» «param.name»«ENDFOR») {
          «IF comp.superComponent.isPresent»
      ...
      public <T> void setPort(String name, Port<T> port){
      	...
      	if (name.equals(«inPort.name»)){
      		setPort«inPort.name.toFirstUpper»((Port<«helper.printPortType(inPort)»>) port);
      	}
      	«ENDFOR»
      }
